...
Scope is fixed until this table is done (owner, 2026-09-15). Order: Exit, A9, then ship. T19 and
T18 are deferred past ship by the owner. Anything else found on the way goes to `docs/BACKLOG.md`,
unless it is a bug a driver would hit or a safety gap.
...
| **A9** | Car downloads: `SongDownloadManager` into a shared module, local-URI resolution in shared code (restore included), parked-only download actions, Downloads screen, Library row | A8 | Implemented on `ek/prd-a9-downloads` — PR pending; measured by the Exit suite with screen 15 in the inventory; device pass in `docs/AAOS_A9_VERIFICATION.md` (entry, empty state and driving lock verified; downloading itself owed — no album in the test catalogue) |
| **Exit** | §12 criteria 2 and 3: one automated measurement of touch targets (76dp) and text contrast (7:1) across the launcher | A1–A8 | Done — measured by CarTouchTargetMeasurementTest and CarTextContrastMeasurementTest (PR #63), every list at each scroll step; the two destructive pairs are the recorded AA exception |
...
| ~~Q5~~ | ~~Does Downloads belong in the custom launcher, given it is content browse rather than settings?~~ **Resolved 2026-08-03:** yes for the `oem` launcher. The screen is part of A9 (moved from A8 by D71); delete/remove actions remain parked-only, and the `playstore` path can expose offline content through the media browse tree if needed later. | — | Closed |
...
1. All 20 screens are implemented and match `docs/aaos-DESIGN.md`. Owner-approved exception
   (2026-09-15): screen 2 (PinOptIn, T18) and screen 1's phone and email sign-in (T19) ship later;
   screen 1 ships with Google sign-in.
